...
pull_Puz() {
    ...
    case $2 in
        ...
        1)  if [[ ! -f "$PuzzDir/Wall_Street_Journal_$1.puz" ]]; then
                wget "https://herbach.dnsalias.com/wsj/wsj$1.puz" -O $PuzzDir/Wall_Street_Journal_$1.puz
            fi
            cursewords $PuzzDir/Wall_Street_Journal_$1.puz 
            ;;
        2)
            if [[ ! -f "$PuzzDir/Univeral_Daily_Crossword_$1.puz" ]]; then
